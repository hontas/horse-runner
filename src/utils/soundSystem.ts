class SoundSystem {
  ...
  private async generateNoiseBuffer(duration: number, filterFreq?: number): Promise<AudioBuffer> {
    if (!this.audioContext) throw new Error('AudioContext not available');
  ...
  async initializeSounds() {
    ...
    try {
      ...
      for (let i = 0; i < jumpData.length; i++) {
        ...
        jumpData[i] = Math.sin(2 * Math.PI * freqSweep * t) * 0.3 * (1 - t / 0.3);
      }
      ...
      const starBuffer = this.audioContext.createBuffer(1, this.audioContext.sampleRate * 0.6, this.audioContext.sampleRate);
      ...
      const gameOverBuffer = this.audioContext.createBuffer(1, this.audioContext.sampleRate * 1.5, this.audioContext.sampleRate);
      ...
      const boostBuffer = this.audioContext.createBuffer(1, this.audioContext.sampleRate * 0.5, this.audioContext.sampleRate);
      ...
      const wallHitBuffer = this.audioContext.createBuffer(1, this.audioContext.sampleRate * 0.4, this.audioContext.sampleRate);
  ...
  playSound(soundName: string, volume: number = 1) {
    if (!this.audioContext || !this.isEnabled || this.audioContext.state === 'suspended') {
      return;
    }
